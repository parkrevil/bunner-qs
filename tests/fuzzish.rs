...
fn seed_allow_cases() {
    ...
    for case in load_cases(DATA) {
        ...
        let result = parse_with_options(&case.input, &opts);
...
proptest! {
    ...
    fn plus_space_transcode(value in unicode_value_string()) {
        ...
        let parsed = parse_with_options(&query, &opts).expect("should decode plus as space");
    ...
    fn max_params_guard(limit in 0usize..6, extra in 1usize..4) {
        ...
        let result = parse_with_options(&query, &opts);
        match result {
