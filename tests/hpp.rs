...
fn assert_array_like(value: &Value, expected: &[&str]) {
    match value {
        Value::Array(arr) => {
        ...
        Value::Object(obj) => {
            ...
            for (idx, expected_value) in expected.iter().enumerate() {
                ...
                assert_eq!(obj.get(&key).unwrap().as_str().unwrap(), *expected_value);
            }
        }
        other => panic!("expected array-like value, got {other:?}"),
    }
}
...
fn allows_uniform_append_pattern() {
    ...
    assert_array_like(key_value, &["1", "2"]);
}
...
fn allows_uniform_numeric_pattern() {
    ...
    assert_array_like(items_value, &["apple", "banana"]);
}
