...
fn allows_uniform_append_pattern() {
    let parsed = parse("key[]=1&key[]=2", None).expect("uniform append pattern should parse");
...
fn allows_uniform_numeric_pattern() {
    let parsed = parse("items[0]=apple&items[1]=banana", None)
        .expect("uniform numeric pattern should parse");
...
fn rejects_mixed_append_and_numeric() {
    let error = parse("key[]=1&key[0]=1", None).expect_err("mixed append/numeric should fail");
    match error {
...
fn rejects_scalar_and_nested_mix() {
    let error = parse("foo=1&foo[bar]=2", None).expect_err("mixing scalar and nested should fail");
    match error {
...
fn rejects_duplicate_scalar_values() {
    let error = parse("foo=1&foo=2", None).expect_err("duplicate scalar keys should fail");
    match error {
        ParseError::DuplicateKey { key } => assert_eq!(key, "foo"),
        other => panic!("unexpected error {other:?}"),
    }
...
fn rejects_non_contiguous_numeric_indexes() {
    let error = parse("items[0]=apple&items[2]=cherry", None)
